...
impl DAO<User, ObjectId> for UserDAO {
    async fn get(oid: ObjectId) -> actix_web::Result<Option<User>> {
        UserDAO::get_collection().await.find_one(
            doc! {
                "_id": oid
            },
            None,
    ...
    async fn insert(user: &mut User) -> actix_web::Result<ObjectId> {
        let insert_result = UserDAO::get_collection().await
    ...
    async fn update(user: &User) -> actix_web::Result<u64> {
        if let Some(id) = user.id {
            let update_result = UserDAO::get_collection().await.update_one(
                doc! {
                "_id": id
            },
                doc! {
        ...
        Err(actix_web::error::ErrorInternalServerError("user id not found"))
    }
